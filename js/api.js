...
const ApiEngine = (function () {
  ...
  /** 验证楼层号仍然存在；第二 API 耗时期间聊天可能已变化。 */
  function validatedMessageId(messageId) {
    // 先用 getChatMessages 确认该楼层仍然存在
    if (typeof getChatMessages === 'function') {
      try {
        const found = getChatMessages(messageId);
        if (Array.isArray(found) && found.length > 0) return messageId;
      } catch (e) { /* 楼层号超出范围，走回退 */ }
    }
    // 回退到当前最新楼层
    if (typeof getLastMessageId === 'function') {
      try {
        const fallback = Number(getLastMessageId());
        if (Number.isFinite(fallback) && fallback >= 0) {
          log(PREFIX_SECOND, '楼层回退', { original: messageId, fallback });
          return fallback;
        }
      } catch (e) { /* ignore */ }
    }
  ...
  async function applyUpdate(messageId, original, baseline, generated) {
    ...
    const safeId = validatedMessageId(messageId);
    await setChatMessages([{ message_id: safeId, message: merged }], { refresh: 'none' });
    await Mvu.replaceMvuData(parsed, { type: 'message', message_id: safeId });
    return merged;
  }
