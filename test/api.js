...
(async () => {
  ...
  if (fs.existsSync(apiPath)) {
    ...
    ok(configs.length === 1 && rawConfigs.length === 0, 'current 模式直接使用 generate');
    ok(!configs[0].preset_name && !configs[0].injects, 'current 模式不传 preset_name 或 injects');
    ...
    ok(requestDiagnostics.some((d) => d.stage === 'normal' && d.mode === 'fixed'
      && d.targetPreset === '变量专用' && d.transport === 'generate-switched'
    ...
    await win.ApiEngine.callSecondApiForVariable({ baseline: win.MVU.getDataSnapshot(), purpose: 'normal' });
    const taskNeedle = '玩家普通变量要求';
    const countText = (value) => (String(value || '').includes(taskNeedle) ? 1 : 0);
    const windowsCopies = countText(hostConfig.user_input)
      + (hostConfig.injects || []).reduce((sum, item) => sum + countText(item.content), 0);
    const termuxCopies = countText(hostConfig.user_input);
    ok(windowsCopies === 1, 'Windows 宿主只收到一份更新任务');
    ...
    await win.ApiEngine.callSecondApiForVariable({ baseline: win.MVU.getDataSnapshot(), purpose: 'normal' });
    ok(repairInputs.length === 2 && repairInputs.every((config) => !config.injects),
      '格式纠正两次请求都不使用 injects');
    ok(/玩家普通变量要求/.test(repairInputs[1].user_input) && /只需修正格式/.test(repairInputs[1].user_input),
      '格式纠正仍通过唯一 user_input 携带完整任务');
