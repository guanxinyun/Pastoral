...
(async () => {
  ...
  ok(!!doc.querySelector('[data-icon-shared^="crop:"]'), '已种田格提供作物共享图标键');
  ...
  const seedIcon = farmPanel.querySelector('[data-icon-target^="crop:"]');
  ok(!!seedIcon && /种子及同名作物/.test(seedIcon.dataset.iconLabel), '种子图鉴与同名作物共享 crop 键');
  ok(seedIcon && seedIcon.dataset.iconTarget === 'crop:春小麦', '种子名去除“种子”后与田格作物键完全一致');
