...
if (fs.existsSync(sourcePath)) {
  ...
  ok(cfg.variablePresets.normal.mode === 'current' && cfg.variablePresets.endday.mode === 'current', '普通与归寝变量请求默认均沿用当前预设');
  ...
  ok(saved.legacy === 7 && !Object.prototype.hasOwnProperty.call(saved, 'apiMode'), '保存时删除旧模式并保留其他未知字段');
  ...
  ok(!Object.prototype.hasOwnProperty.call(saved.variablePresets.normal, 'context')
    && !Object.prototype.hasOwnProperty.call(saved.variablePresets.endday, 'context'), '保存时清除两阶段旧上下文字段');
  ...
  const emptyIssues = win.Settings.secondApiIssues({ secondApi: { url: '', key: '', model: '' } });
  ok(emptyIssues.join(',') === 'URL,API Key,模型', '空第二 API 明确报告 URL、Key、模型三项缺失');
  ...
  ok(malformed.variablePresets.normal.mode === 'current' && malformed.variablePresets.normal.presetName === '123', '非法预设模式迁移 current 并规范化预设名');
  ok(!Object.prototype.hasOwnProperty.call(malformed.variablePresets.normal, 'context'), '损坏设置中的旧上下文字段被删除');
  ok(malformed.variablePresets.endday.mode === 'current', '损坏的归寝预设设置回退当前预设');
...
if (fs.existsSync(sourcePath)) {
  ...
  ok(!Object.prototype.hasOwnProperty.call(migrated.variablePresets.normal, 'assembly')
    && !Object.prototype.hasOwnProperty.call(migrated.variablePresets.endday, 'assembly'),
  '旧 assembly 设置迁移后不再参与运行');
  ok(migrated.variablePresets.normal.presetName === '日常A'
    && migrated.variablePresets.endday.presetName === '归寝B', '迁移不混淆两阶段预设');
  ok(migrated.variablePresets.normal.temperature === 0.2
    && migrated.variablePresets.endday.temperature === 0.4, '迁移保留两阶段独立温度');
